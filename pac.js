...
const {log, error} = require("./comm.js");
...
function listfile(req, res) {
  ...
  dirs.forEach(f=>{
    p('<li><a href="getfile?file=', f, '">', f, '</a> <a href="testfile?file=', f, '&url=facebook.com">[Open]</a></li>');
  });
  p.end();
}
...
function savefile(req, res) {
  readAllText(req).then(txt=>{
    ...
    res.writeHead(200, { 'Content-Type': 'text/html' });
    res.end('<div>ok, '+ file +"</div> <a href='/'>return</a>");
  }).catch(err=>{
